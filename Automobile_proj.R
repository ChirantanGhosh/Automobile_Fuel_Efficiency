...
vehicles <- read.csv("C:\\Users\\Chirantan\\Desktop\\ds\\vehicles.csv")
head(vehicles)

labels <- read.table("C:\\Users\\Chirantan\\Desktop\\ds\\Automobile_data_Description.txt", sep = "-", header =
                       FALSE)

labels <- do.call(rbind, strsplit(readLines("C:\\Users\\Chirantan\\Desktop\\ds\\Automobile_data_Description.txt"),
                                  " - "))

head(labels)

x <- readLines("C:\\Users\\Chirantan\\Desktop\\ds\\Automobile_data_Description.txt")
y <- strsplit(x, " - ")
...
byYear <- ddply(gasCars, ~year, summarise, avgMPG =
 ...
 commonMakes <- Reduce(union, uniqMakes)
 commonMakes
